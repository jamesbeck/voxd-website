...
function renderPrimitiveValue(value: unknown) {
  ...
  if (typeof value === "string") {
    return <ValueSurface className="break-words whitespace-pre-wrap">{value}</ValueSurface>;
  }
  ...
  return (
    <div className={cn("space-y-1", compact && "space-y-0.5")}>
      <div className="flex flex-wrap items-center gap-2">
        <h4 className={cn("font-medium text-foreground", !compact && "text-lg")}>{label}</h4>
        {fieldKey && (
...
function ComplexSchemaField({
  ...
  return (
    <div className="space-y-2 border-b pb-4 last:border-b-0 last:pb-0">
      <SchemaFieldHeader fieldKey={fieldKey} schema={schema} compact={depth > 0} />
        ...
        <div className="rounded-lg border border-border/70 bg-muted/10 transition-colors">
          <CollapsibleTrigger className="w-full">
            <div className="flex items-center justify-between gap-3 p-4 text-left">
              <p className="text-sm font-medium text-foreground">{fieldCountLabel}</p>
              {isOpen ? (
...
function SchemaValue({
  ...
  if (Array.isArray(data)) {
    ...
    return (
      <Card className={cn(depth > 0 && "shadow-none", !showHeader && "gap-0 border-0 bg-transparent p-0 shadow-none")}>
        {showHeader && (
          <CardHeader>
            <SchemaFieldHeader fieldKey={fieldKey} schema={schema} compact={depth > 0} />
          </CardHeader>
            ...
            <div className="space-y-3">
              {data.every(
                ...
                <ValueSurface className="flex flex-wrap gap-2 whitespace-pre-wrap">
                  {data.map((item, index) => (
                    <span key={`${fieldKey || "item"}-${index}`} className="rounded border border-border/60 bg-background/80 px-2 py-1">
              ...
              {entries.map(([key, value]) => {
                ...
                return (
                  <div key={key} className="space-y-2 border-b pb-4 last:border-b-0 last:pb-0">
                    <SchemaFieldHeader fieldKey={key} schema={propertySchema} compact={depth > 0} />
...
export default function JsonSchemaDataViewer({
  ...
  return (
    <SchemaValue data={data} schema={schema} depth={0} />
  );
}