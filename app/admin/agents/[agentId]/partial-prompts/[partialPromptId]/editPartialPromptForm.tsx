...
export default function EditPartialPromptForm({
  partialPromptId,
  agentId,
  name,
  text,
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const response = await saUpdatePartialPrompt({
      partialPromptId,
      name: values.name,
      text: values.text,
    });
    ...
    if (!response.success) {
      ...
      toast.error("There was an error updating the partial prompt");
    ...
    if (response.success) {
      toast.success("Partial prompt saved");
      router.refresh();
    }
