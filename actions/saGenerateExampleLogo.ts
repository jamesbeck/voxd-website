...
Write a brief logo prompt (max 1 sentence). Include company name and key visual elements only.`,
    });

    const logoPrompt = promptGenerationResult.text.trim();

    // Add white background requirement
    const finalPrompt = `Simple professional logo for ${
      example.businessName || "the business"
    }. ${logoPrompt}. Plain white background, minimal padding, the logo should nearly reach the edges of the image, centered.`;
    ...
    try {
      ...
      // threshold: 10 means colors within 10 units of white (255,255,255) will be considered "background"
      const paddingAmount = 30; // pixels of padding on all sides

      const processedBuffer = await sharp(buffer)
        .trim({
          threshold: 10, // Adjust if needed - higher values trim more aggressively
        })
        .extend({
          top: paddingAmount,
      ...
      console.log("Logo cropped and padded successfully");
    } catch (trimError) {
      console.error("Error trimming logo, using original:", trimError);
      // If trimming fails, continue with the original buffer
    }
